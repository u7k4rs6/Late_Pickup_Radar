...
| 5 | 1:28 | T1 | The five metrics for the sample: it gives 9.96% against 10.02% on the full month, within 0.06 points. Then the decision lists, which come from the committed full-month run, because a sample cannot fill two hundred trips per cell. | point at the two lists |
